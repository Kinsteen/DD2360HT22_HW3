...
__global__ void mover_PC_kernel(struct particles *part, struct EMfield* field, struct grid* grd, struct parameters* param) {
    ...
    // Are those independent particle to particle?
    // auxiliary variables
    FPpart dt_sub_cycling = (FPpart) param->dt/((double) part->n_sub_cycles);
    FPpart dto2 = .5*dt_sub_cycling, qomdt2 = part->qom*dto2/param->c;
    FPpart omdtsq, denom, ut, vt, wt, udotb;
...
int mover_PC_gpu(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param)
{
    ...
    struct grid* dev_grd;
    struct parameters* dev_param;

    cudaMalloc(&dev_part, sizeof(particles));
    cudaMalloc(&dev_field, sizeof(EMfield));
    cudaMalloc(&dev_grd, sizeof(grid));
    cudaMalloc(&dev_param, sizeof(parameters));

    cudaMemcpy(dev_part, part, sizeof(particles), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_field, field, sizeof(EMfield), cudaMemcpyHostToDevice); // Not necessary?
    cudaMemcpy(dev_grd, grd, sizeof(grid), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_param, param, sizeof(parameters), cudaMemcpyHostToDevice);

    // copy particules
    FPpart* dev_x; FPpart*  dev_y; FPpart* dev_z; FPpart* dev_u; FPpart* dev_v; FPpart* dev_w;
    ...
    cudaMemcpy(dev_x, part->x, part->npmax * sizeof(FPpart), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_y, part->y, part->npmax * sizeof(FPpart), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_z, part->z, part->npmax * sizeof(FPpart), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_u, part->u, part->npmax * sizeof(FPpart), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_v, part->v, part->npmax * sizeof(FPpart), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_w, part->w, part->npmax * sizeof(FPpart), cudaMemcpyHostToDevice);

    cudaMemcpy(&(dev_part->x), &dev_x, sizeof(FPpart *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_part->y), &dev_y, sizeof(FPpart *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_part->z), &dev_z, sizeof(FPpart *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_part->u), &dev_u, sizeof(FPpart *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_part->v), &dev_v, sizeof(FPpart *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_part->w), &dev_w, sizeof(FPpart *), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(&(dev_field->Ex_flat), &Ex_flat, sizeof(FPfield *), cudaMemcpyHostToDevice); // not dev to dev??
    cudaMemcpy(&(dev_field->Ey_flat), &Ey_flat, sizeof(FPfield *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_field->Ez_flat), &Ez_flat, sizeof(FPfield *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_field->Bxn_flat), &Bxn_flat, sizeof(FPfield *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_field->Byn_flat), &Byn_flat, sizeof(FPfield *), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_field->Bzn_flat), &Bzn_flat, sizeof(FPfield *), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(&(dev_grd->XN_flat), &XN_flat, sizeof(FPfield*), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_grd->YN_flat), &YN_flat, sizeof(FPfield*), cudaMemcpyHostToDevice);
    cudaMemcpy(&(dev_grd->ZN_flat), &ZN_flat, sizeof(FPfield*), cudaMemcpyHostToDevice);
    
    // start subcycling
    for (int i_sub=0; i_sub <  part->n_sub_cycles; i_sub++) {
        int TPB = 1024;
        int blocks = ceil(part->nop / (float) TPB);
        printf("Blcoks: %d\n", blocks);
        // launch kernel for one particule
        mover_PC_kernel<<<blocks,TPB>>>(dev_part, dev_field, dev_grd, dev_param);
    }

    cudaDeviceSynchronize();

    cudaMemcpy(part, dev_part, sizeof(particles), cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(host_x, dev_x, part->npmax * sizeof(FPpart), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_y, dev_y, part->npmax * sizeof(FPpart), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_z, dev_z, part->npmax * sizeof(FPpart), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_u, dev_u, part->npmax * sizeof(FPpart), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_v, dev_v, part->npmax * sizeof(FPpart), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_w, dev_w, part->npmax * sizeof(FPpart), cudaMemcpyDeviceToHost);
